...
import com.github.cc007.trafficlights.tools.ToolListener;
...
public class CurveUtils {
    ...
    /**
     * Calculate the intersection coordinats for two lines, given a base point
     * and angle for each line.
     *
     ...
     *
     * public static QCurve createCurve(Point p1, Point p2, double a1, double
     * a2) throws CurveException { Point tp = new Point((int)(p1.x +
     * p1.distance(p2) * Math.cos(a1)),(int)(p1.y - p1.distance(p2) *
     * Math.sin(a1))); Rectangle r = new Rectangle(p2.x - 10, p2.y - 10, 20,
     * 20); Point ip; if(r.contains(tp)) return new QCurve(p1,p2); ip =
     * calcIntersect(p1,p2,a1,a2); QCurve c = new QCurve(p1,ip,p2); // test voor
     * path-iterator-bug: Point2D.Double p = c.next(c.getFirst(),2);
     * if(!(c.getBounds().contains(p))) { c = new QCurve(p2,ip,p1); p =
     * c.next(c.getFirst(),2); c.setSwitched(true); } return c; }
     *
     *
     * /**
    ...
    /**
     * Returns the coordinates of the point with specified index and stepsize on
     * a given curve
     *
     * @param c The curve
     * @param step The stepsize
    ...
    public static void setupRoadSizes(Road road, int step) throws InfraException {
        ...
        try {
            ...
            Logger.getLogger(CurveUtils.class.getName()).log(Level.SEVERE, null, ce);
        }
    ...
    public static int getRoadLength(Road road, int step) throws InfraException {
        ...
        try {
            ...
            Logger.getLogger(CurveUtils.class.getName()).log(Level.SEVERE, null, exc);
