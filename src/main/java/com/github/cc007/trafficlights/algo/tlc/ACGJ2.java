...
/**
 * This algorithm will, when it starts, handle like Longest Queue, but it can do
 * more things: 1. It prevents infinite waiting of Roadusers 2. It tries to
 * create green waves between busy nodes. The green_wave-factor is editable as a
 * parameter. 3. The algorithm can be stimulated to develop patterns in
 * TL-configuration-settings, therefore it remembers the last 100
 * configurations. This pattern factor is also editable 4. This algorithm has a
 * keep green factor. This editable factor is an extra weight to keep the lights
 * green, that were green in the previous cycle.
 */
public class ACGJ2 extends TLController implements XMLSerializable, TwoStageLoader, InstantiationAssistant {
    ...
    private class NextCycles implements XMLSerializable {
        ...
        @Override
        public void saveChilds(XMLSaver saver) throws XMLTreeException, IOException, XMLCannotSaveException { 	// A NextCycles object has no child objects
        }
    ...
    public Object createInstance(Class request) throws
        ...
        if (NextCycles.class.equals(request)) {
            ...
            throw new ClassNotFoundException("ACGJ2 InstantiationAssistant cannot make instances of "
                    + request);
        }
    }
