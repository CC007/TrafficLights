...
public class GenNeuralTLC extends TLController implements XMLSerializable, TwoStageLoader, InstantiationAssistant {
     ...
     * and neural-network techniques to find an optimum in calculating the gains
     * for each trafficlight. Till date it hasnt functioned that well. Rather
    ...
    protected class GenNeuralPopulation implements XMLSerializable, TwoStageLoader {
        ...
        /**
         * BubbleSorts an array of GenNeuralIndividuals on the parameter of
         * performance
         *
        ...
        /**
         * Evolves the current generation of GenNeuralIndividuals in the
         * GenNeuralPopulation into a new one.
         */
        ...
        public void setParentName(String parentName_) throws XMLTreeException {
            throw new XMLTreeException("Operation not supported. GenNeuralPopulation has a fixed parentname");
        }

        // GenNeuralPopulation TwoStageLoader implementation
        @Override
        public void loadSecondStage(Map maps) throws XMLTreeException, XMLInvalidInputException {
            List<GenNeuralIndividual> tmp = new ArrayList<>();
            for (int i = 0; i < inds.length; i++) {
                for (int j = 0; j < inds[i].length; j++) {
                    tmp.add(inds[i][j]);
                }
            }
            XMLUtils.loadSecondStage(tmp, maps);
        }
    ...
    public Object createInstance(Class request) throws
        ...
        if (GenNeuralPopulation.class.equals(request)) {
            ...
            throw new ClassNotFoundException("GenNeural InstantiationAssistant cannot make instances of "
                    + request);
        }
    }
