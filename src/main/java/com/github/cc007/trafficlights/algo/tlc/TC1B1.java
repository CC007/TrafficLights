 ...
 * learn how to alter it's outcome by reinforcement learning. Now Bucketed Now
 * Optimized 2.0
 *
 * @author Arne K, Jilles V
 * @version 0.915
 */
public class TC1B1 extends TCRL implements Colearning, InstantiationAssistant {
    // TLC vars
...
avg wT:			V([tl,p,d]) = Sum(L) [P([tl,p,d],L)Q([tl,p,d],L)]
    ...
    protected void recalcP(int tlId, int pos, int desId, boolean light, int tlNewId, int posNew) {
        ...
        if (count_index >= 0) {
            ...
            CountEntry temp = (CountEntry) count[tlId][pos][desId].get(count_index);
            temp.incrementValue();
        ...
        while (it.hasNext()) {
            CountEntry enumelem = (CountEntry) it.next();
    ...
    protected void recalcQ(int tlId, int pos, int desId, boolean light, int tlNewId, int posNew, PosMov[] posMovs, int penalty) {
 ...
 /*
    ...
    public class PEntry implements XMLSerializable {
        ...
        public void setSameStartSituation(int s) {
            sameStartSituation = s;
        }

        public void setSameSituation(int s) {
            sameSituation = s;
        }
    ...
    public Object createInstance(Class request) throws
        ...
        if (CountEntry.class.equals(request)) {
            ...
            throw new ClassNotFoundException("TC1 IntstantiationAssistant cannot make instances of "
                    + request);
        }
    }
}
