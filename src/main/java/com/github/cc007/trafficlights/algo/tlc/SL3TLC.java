 ...
 *
 * Changed to use Sarsa(0) with bus priorities instead of DP - S. Louring
 *------------------------------------------------------------------------*/
 ...
 * @version 1.1
 */
public class SL3TLC extends TCRL implements InstantiationAssistant {
    ...
    protected final static String shortXMLName = "tlc-sl3";
     ...
     */
    public SL3TLC(Infrastructure infra) throws InfraException {
        super(infra);
            ...
            for (int j = 0; j < num_dec; j++) {
                ...
                if (!do_this_random) {
                    for (; waitingsize > 0; waitingsize--) {
                        ...
                        // And here are the additions to the code
                        if (ru.getType() == 2) {
                            gain += 2 * (q_table[currenttl.getId()][pos][destination.getId()][1] - q_table[currenttl.getId()][pos][destination.getId()][0]);  //red - green
                        } else {
...
//OBS       // Her burde laves et tjek p� om 0 < gamma < 1 og det samme med random chance
    ...
    public void loadSecondStage(Map<String, Map<Integer, TwoStageLoader>> maps) throws XMLInvalidInputException, XMLTreeException {
        ...
        //XMLUtils.loadSecondStage(p_table,maps);
        System.out.println("SL3 second stage load finished.");
    }
    ...
    public Object createInstance(Class request) throws
        ...
        if (CountEntry.class.equals(request)) {
            ...
            throw new ClassNotFoundException("SL3 IntstantiationAssistant cannot make instances of "
                    + request);
        }
    }
