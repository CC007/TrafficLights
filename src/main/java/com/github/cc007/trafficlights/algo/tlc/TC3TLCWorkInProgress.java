 ...
 * @version 0.1
 */
public class TC3TLCWorkInProgress extends TCRL implements Colearning, InstantiationAssistant {
    // TLC vars
    ...
    public void updateRoaduserMove(Roaduser ru, DriveLane prevlane, Sign prevsign, int prevpos, DriveLane dlanenow, Sign signnow, int posnow, PosMov[] posMovs, DriveLane desired, int penalty) {
        ...
        if (prevsign.getType() == Sign.TRAFFICLIGHT && (signnow.getType() == Sign.TRAFFICLIGHT || signnow.getType() == Sign.NO_SIGN)) {
            ...
            recalcQa(tlId, prevpos, desId, prevsign.mayDrive(), signnow.getId(), posnow, posMovs, penalty);
            recalcQ(tlId, prevpos, desId, prevsign.mayDrive(), signnow.getId(), posnow, posMovs, Ktl, penalty);
        }
    }

    protected void recalcP(int tlId, int pos, int desId, boolean light, int tlNewId, int posNew, int Ktl) {	// Meneer Kaktus is tevree!
        // Is OK.
    ...
    protected void recalcV(int tlId, int pos, int desId, boolean light, int Ktl, PosMov[] posMovs) {	// Meneer Kaktus is Tevree!
    ...
    protected void recalcQa(int tlId, int pos, int desId, boolean light, int tlNewId, int posNew, PosMov[] posMovs, int penalty) {	// Meneer Kaktus is Tevree!
        // Q'([tl,p,d],L) = Sum(p') [P([tl,p,d],L,*,[tl,p']) * (R([tl,p],[tl,p'])+yW([tl,p',d]))]
        // p' = alle mogelijke posities voor me op deze lane

        float Qa = penalty, W = 0, P = 0, R = 0;
    ...
    protected void recalcQ(int tlId, int pos, int desId, boolean light, int tlNewId, int posNew, PosMov[] posMovs, int Ktl, int penalty) {	// Meneer Kaktus is Tevree!
        ...
        qTable[tlId][pos][desId][light ? green_index : red_index] = Qa + +penalty + sumTLP;
    }
