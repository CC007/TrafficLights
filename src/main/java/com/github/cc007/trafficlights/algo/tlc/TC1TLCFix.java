 ...
 * learn how to alter it's outcome by reinforcement learning. Now being fixed.
 *
 * @author Arne K, Jilles V
 * @version 2.0
 */
public class TC1TLCFix extends TCRL implements Colearning, InstantiationAssistant {	// TLC vars
    ...
    public void setInfrastructure(Infrastructure infra) {
        ...
        try {
            ...
            Logger.getLogger(TC1TLCFix.class.getName()).log(Level.SEVERE, null, e);
        }
...
//					System.out.println("node: "+i+" light: "+j+" gain: "+gain+" "+targets[0]+" "+targets[1]+" "+targets[2]+" "+currentlane2.getNumRoadusersWaiting());
    ...
    public void updateRoaduserMove(Roaduser ru, DriveLane prevlane, Sign prevsign, int prevpos, DriveLane dlanenow, Sign signnow, int posnow, PosMov[] posMovs, DriveLane desired, int penalty) {
        ...
        if (prevsign.getType() == Sign.TRAFFICLIGHT && (signnow.getType() == Sign.TRAFFICLIGHT || signnow.getType() == Sign.NO_SIGN)) {
            ...
            recalcP(tlId, prevpos, desId, prevsign.mayDrive(), signnow.getId(), posnow, ru, dlanenow);
            ...
            if (ru.getInQueueForSign()) {
                recalcP(ru.getPrevWaitTl(), ru.getPrevWaitPos(), desId, ru.getPrevWaitTlColor(), signnow.getId(), posnow, ru, dlanenow);
                recalcQ2(ru.getPrevWaitTl(), ru.getPrevWaitPos(), desId, ru.getPrevWaitTlColor(), signnow.getId(), posnow, ru, dlanenow, posMovs);
                recalcV(ru.getPrevWaitTl(), ru.getPrevWaitPos(), desId);
 ...
 /* Nieuw
			    recalcP(ru.getPrevWaitTl(), ru.getPrevWaitPos(), desId, ru.getPrevWaitTlColor(), signnow.getId(), posnow, ru, dlanenow);
			    recalcQ(ru.getPrevWaitTl(), ru.getPrevWaitPos(), desId, ru.getPrevWaitTlColor(), signnow.getId(), posnow, ru, dlanenow);
			    recalcV(ru.getPrevWaitTl(), ru.getPrevWaitPos(), desId);
                 */
 /*
    			recalcP(tlId, prevpos, desId, prevsign.mayDrive(), signnow.getId(), posnow, ru, dlanenow);
    ...
    protected void recalcP(int tlId, int pos, int desId, boolean light, int tlNewId, int posNew, Roaduser ru, DriveLane dl) {
        ...
        if (true) {
            ...
            for (int i = 0; i < num_c; i++) {
                curC = count[tlId][pos][desId].get(i);
    ...
    protected void recalcQ(int tlId, int pos, int desId, boolean light, int tlNewId, int posNew, Roaduser ru, DriveLane dl, int penalty) {
        ...
        if ((tlId == ru.getCurrentWaitTl()) && (tlId != tlNewId || posNew == ru.getCurrentWaitPos())) {
            ...
            qTable[tlNewId][posNew][desId][light ? green_index : red_index] = Q;
        }
    }
    ...
    protected void recalcQOpt(int tlId, int pos, int desId, boolean light, int tlNewId, int posNew, PosMov[] posMovs) {	// Meneer Kaktus zegt: OK!
    ...
    @Override
    public boolean canCreateInstance(Class request) {// 	System.out.println("Called TC1TLC-opt instantiation assistant ??");
    ...
    public Object createInstance(Class request) throws
            ClassNotFoundException, InstantiationException, IllegalAccessException { //	System.out.println("Called TC1TLC-opt instantiation assistant");
