 ...
 * @version 1.0
 */
public class TC3B1 extends TCRL implements Colearning, InstantiationAssistant {
    // TLC vars
     ...
     */
    public TC3B1(Infrastructure infra) throws InfraException {
        super(infra);
    }
    ...
    public void setInfrastructure(Infrastructure infra) {
        ...
        try {
            ...
            p_table = new ArrayList[numSigns][][];
            pKtl_table = new ArrayList[numSigns][][];
            ...
            for (int i = 0; i < num_nodes; i++) {
                ...
                for (int j = 0; j < num_dls; j++) {
                    ...
                    p_table[id] = new ArrayList[num_pos_on_dl][];
                    pKtl_table[id] = new ArrayList[num_pos_on_dl][];
                    ...
                    for (int k = 0; k < num_pos_on_dl; k++) {
                        ...
                        p_table[id][k] = new ArrayList[num_specialnodes];
                        pKtl_table[id][k] = new ArrayList[num_specialnodes];
    ...
    public TLDecision[][] decideTLs() {
        ...
        for (int i = 0; i < num_nodes; i++) {
            ...
            for (int j = 0; j < num_dec; j++) {
                ...
                bucket[currenttlID][BUCK] = gain;
                bucket[currenttlID][WAIT] = waitingsize;
            }
        }
        return tld;
    }
    ...
    public void updateRoaduserMove(Roaduser ru, DriveLaneTemp prevlane, Sign prevsign, int prevpos, DriveLaneTemp dlanenow, Sign signnow, int posnow, PosMov[] posMovs, DriveLaneTemp desired, int penalty) {
        ...
        if (prevsign.getType() == Sign.TRAFFICLIGHT && (signnow.getType() == Sign.TRAFFICLIGHT || signnow.getType() == Sign.NO_SIGN)) {
            ...
            recalcQa(prevsign, prevpos, dest, light, signnow, posnow, posMovs, penalty);
            recalcQ(prevsign, prevpos, dest, light, signnow, posnow, posMovs, Ktl, penalty);
        }
    ...
    /**
     * Empties the 'gain-value' bucket partly, which is being filled when
     * Roadusers are waiting/voting for their TrafficLight to be set to green.
     *
    ...
    /**
     * When a Roaduser may drive according to it's Sign, but cant as the desired
 DriveLaneTemp is full, (some/all) of the current gain-bucket of it's current
 TrafficLight is siphoned over to the desired DriveLaneTemp, making it more
 probable that that lane will move, making it possible for this DriveLaneTemp
 to move.
     *
    ...
    protected void recalcP(Sign tl, int pos, Node destination, boolean light, Sign tl_new, int pos_new, int Ktl) {
        ...
        Iterator it = count[tlId][pos][desId].iterator();
        while (it.hasNext()) {
            CountEntry current = (CountEntry) it.next();
            dest += current.sameSourceDifferentKtl(currentsituation);
            source += current.sameSource(currentsituation);
        }

        if (source == 0) {
            currentchance.setValue(0);
        } else {
            currentchance.setValue((float) dest / (float) source);
        }

        int p_index = p_table[tlId][pos][desId].indexOf(currentchance);
        if (p_index >= 0) {
            p_table[tlId][pos][desId].set(p_index, currentchance);
        } else {
        ...
        for (; size >= 0; size--) {
            PEntry P = (PEntry) p_table[tlId][pos][desId].get(size);
        ...
        it = count[tlId][pos][desId].iterator();
        while (it.hasNext()) {
            source += ((CountEntry) it.next()).sameSourceWithKtl(currentsituation);
        }

        dest = currentsituation.getValue();
        if (source == 0) {
            currentchance2.setValue(0);
        } else {
            currentchance2.setValue((float) dest / (float) source);
        }

        p_index = pKtl_table[tlId][pos][desId].indexOf(currentchance2);
        if (p_index >= 0) {
            pKtl_table[tlId][pos][desId].set(p_index, currentchance2);
        } else {
        ...
        for (; size >= 0; size--) {
            PKtlEntry P = (PKtlEntry) pKtl_table[tlId][pos][desId].get(size);
            float pvalue = P.sameSource(currentsituation);
            if (pvalue > -1) {
                if (size != p_index) {
                    P.setValue(pvalue * (float) (source - 1) / (float) source);
        ...
        if (currentchance.getValue() > 1 || currentchance2.getValue() > 1 || currentchance.getValue() < 0 || currentchance2.getValue() < 0) {
            System.out.println("Serious error !!!!!!!!!1");
        }
    }

    protected void recalcQ(Sign tl, int pos, Node destination, boolean light, Sign tl_new, int pos_new, PosMov[] posMovs, int Ktl, int penalty) {
        /* The calculation of the Q values in TC-3 */
        float newQvalue = penalty + qa_table[tl.getId()][pos][destination.getId()][light ? green_index : red_index];
        float V = 0;

// Waarom splitst TC2 wel op rood/groen, en TC3 niet??
        CountEntry currentsituation = new CountEntry(tl, pos, destination, light, tl_new, pos_new, Ktl);
        Iterator it = pKtl_table[tl.getId()][pos][destination.getId()].iterator();

        while (it.hasNext()) {
            PKtlEntry P = (PKtlEntry) it.next();
...
// Moet er hier geen reward functie??
    ...
    protected void recalcV(Sign tl, int pos, Node destination, boolean light, int Ktl) {
        ...
        Iterator it = pKtl_table[tlId][pos][desId].iterator();
    ...
    protected void recalcQa(Sign tl, int pos, Node destination, boolean light, Sign tl_new, int pos_new, PosMov[] posMovs, int penalty) {
        float newQvalue = penalty;
        ...
        for (; size >= 0; size--) {
            ...
            if (p_index >= 0) {
                try {
                    P = (PEntry) p_table[tlId][pos][desId].get(p_index);
    ...
    protected int[] count(Sign tl, int pos, Node destination) {
        ...
        for (; psize >= 0; psize--) {
            PEntry candidate = (PEntry) p_table[tlId][pos][desId].get(psize);
    ...
    public float getColearnValue(Sign sign_new, Sign sign, Node destination, int pos) {
        ...
        for (; size >= 0; size--) {
            ...
            if (p_index >= 0) {
                try {
                    P = (PKtlEntry) pKtl_table[tlId][pos][desId].get(p_index);
    ...
    public class CountEntry implements XMLSerializable, TwoStageLoader {
        ...
        public boolean equals(Object other) {
            if (other != null && other instanceof CountEntry) {
                ...
                if (!countnew.tl.equals(tl)) {
                    return false;
                }
                if (countnew.pos != pos) {
                    return false;
                }
                if (!countnew.destination.equals(destination)) {
                    return false;
                }
                if (countnew.light != light) {
                    return false;
                }
                if (!countnew.tl_new.equals(tl_new)) {
                    return false;
                }
                if (countnew.pos_new != pos_new) {
                    return false;
                }
                if (countnew.Ktl != Ktl) {
                    return false;
    ...
    public class PEntry implements XMLSerializable, TwoStageLoader {
        ...
        public boolean equals(Object other) {
            if (other != null && other instanceof PEntry) {
                ...
                if (!pnew.tl.equals(tl)) {
                    return false;
                }
                if (pnew.pos != pos) {
                    return false;
                }
                if (!pnew.destination.equals(destination)) {
                    return false;
                }
                if (pnew.light != light) {
                    return false;
                }
                if (!pnew.tl_new.equals(tl_new)) {
                    return false;
                }
                if (pnew.pos_new != pos_new) {
                    return false;
                }
                return true;
            }
    ...
    public class PKtlEntry implements XMLSerializable, TwoStageLoader {
        ...
        PKtlEntry() {	// Empty constructor for loading
        }
    ...
    public Object createInstance(Class request) throws ClassNotFoundException, InstantiationException, IllegalAccessException {
        if (CountEntry.class.equals(request)) {
            ...
            throw new ClassNotFoundException("TC3 IntstantiationAssistant cannot make instances of "
                    + request);
        }
    }
