...
public class TC3B1 extends TCRL implements Colearning,InstantiationAssistant
{
	...
	*/
	...
	protected void recalcP(Sign tl, int pos, Node destination, boolean light, Sign tl_new, int pos_new, int Ktl)
	{
		...
		Enumeration enumr = count[tlId][pos][desId].elements();
		while(enumr.hasMoreElements()) {
			CountEntry current = (CountEntry) enumr.nextElement();
		...
		enumr = count[tlId][pos][desId].elements();
		while(enumr.hasMoreElements()) {
			source += ((CountEntry) enumr.nextElement()).sameSourceWithKtl(currentsituation);
		}
