 ...
 *
 * Changed to use Sarsa(0) with a speciela calculation of gain instead
 * of DP - S. Louring
 *------------------------------------------------------------------------*/
 ...
 * @version 1.1
 */
public class SL4TLC extends TCRL implements InstantiationAssistant {
    ...
    protected final static String shortXMLName = "tlc-sl4";
    ...
    public SL4TLC(Infrastructure infra) throws InfraException {
        super(infra);
            ...
            for (int j = 0; j < num_dec; j++) {
                ...
                if (!do_this_random) {
                    for (; waitingsize > 0; waitingsize--) {
                        ...
                        tempGain = q_table[currenttl.getId()][pos][destination.getId()][1] - q_table[currenttl.getId()][pos][destination.getId()][0]; //red - green
                        gain += (tempGain * tempGain);  //(red - green)^2
                    }
                    float q = gain;
                } else {
...
//OBS       // Her burde laves et tjek p� om 0 < gamma < 1 og det samme med random chance
    ...
    public Object createInstance(Class request) throws
        ...
        if (CountEntry.class.equals(request)) {
            ...
            throw new ClassNotFoundException("SL4 IntstantiationAssistant cannot make instances of "
                    + request);
        }
    }
