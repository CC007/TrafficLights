 ...
 * @version 1.0
 *
 * This class generates CustomRoadusers. This first version supports vehicles,
 * drivers and passengers. However, it only really uses vehicles and drivers;
 * passengers are just an added extra. It uses the vehicle ID and driver ID as
 * the primary key. So if you want to create a unique custom, you need to give
 * it a unique driver and vehicle.
 *
 * A vehicle has a speed, length, name and determines the roaduser type. The
 * speed can be modified by the driver, but not the length, since trafficrules
 * do not allow a car to become longer than x meters.
 *
 * A driver has a name, and it an modify the speed of the vehicle. For example,
 * an old lady will modify the speed of a bicycle by -1. An aggresive truck
 * driver might modify the speed of a truck by +2.
 *
 * The SupportedTypes array maps every RoaduserFactory type that is supported to
 * a Custom index. The TypeVehicle array maps every Custom type index to an
 * array of Vehicle types.
 *
 * The VehicleName array gives every vehicle type a name. The VehicleDriver
 * array maps every vehicle type to a list of possible drivers. From this list
 * is then a driver picked with a chance determined by the PersonChance table.
 * The VehiclePassenger table maps every vehicle type to a list of possible
 * passengers. From this list are then picked a few passengers. If it is null,
 * no passengers are allowed.
 *
 * The VehicleProperties table has the following meaning: - Chance this vehicle
 * has to appear in the sim. - Number of these vehicles currently in the sim. -
 * Maximum number of these vehicles in the sim, 0 is unlimited. - Length in
 * blocks of the vehicle. - Graphical length of the vehicle in pixels. -
 * Graphical width of the vehicle in pixels. - Speed of the vehicle in blocks. -
 * Maximum number of passengers. - Roaduser type.
 *
 * The PersonName array gives every person a name. Might be useful :-)
 *
 * The PersonProperties have the following meaning: - Chance this person has to
 * appear in the sim. - Number of these people currently in simulation - Maximum
 * number of these people in sim, 0 is unlimited. Set to 1 to create unique
 * person. - Speed modifier.
 *
 * The VehicleDriverName table gives every vehicle, with every possible driver a
 * name. If any part is null, a default name is generated using the vehicle name
 * and driver name.
 *
 * The VehicleDriverDescription table gives every vehicle, with every possible
 * driver a description. If any part is null, a default description is generated
 * using the vehicle name, driver name, passenger names, current speed and
 * position on the map.
 *
 * The VehicleDriverPicture table gives every vehicle, with every possible
 * driver a picture. If any part is null, a default picture is used.
 *
 * The VehicleDriverSound table gives every vehicle, with every possible driver
 * an audio clip. If any part is null, a default clip is used.
 *
 * @author Group Datastructures
 * @version 0.02
 *
...
public class CustomFactory {
    ...
    protected static final boolean useOnlyNormalCars = true;
    ...
    public static Roaduser genRoaduser(int type, Node start, Node dest, int pos) throws InfraException {
        ...
        if (ti == -1) {
            return null; // unsupported roaduser type
        }
     ...
     */
    public static String getVehicleName(int vehicle) {
