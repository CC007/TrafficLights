...
public class Road implements Selectable, XMLSerializable, TwoStageLoader {
    ...
    /**
     * Container object to 1tranfer information from the first stage loader to
 ...
 /*============================================*/
    ...
    /**
     * Returns the name of this road. The name of a road is unique in an
     * infrastructure.
     */
 ...
 /*============================================*/
    ...
    public void addTurn(Turn t) throws InfraException {
        if (t == null) {
            throw new InfraException("Parameter t is null");
        }
        turns = (Turn[]) Arrayutils.add(turns, t);
    }
    ...
    public void remTurn(Turn t) throws InfraException {
        ...
        if (i == -1) {
            throw new InfraException("Turn is not part of this road");
        }
    ...
    public void addLane(DriveLaneTemp lane, Node to) throws InfraException {
        ...
        if (to == null) {
            throw new InfraException("Parameter to is null");
        }
        if (to == alphaNode) {
            addAlphaLane(lane);
        } else if (to == betaNode) {
 ...
 /*============================================*/
    /**
     * Returns the other node this road is connected to, seen from the given
     * node
     */
    public Node getOtherNode(Node seenfrom) {
        return seenfrom == alphaNode ? betaNode : alphaNode;
    }
    ...
    public int getNumAllLanes() {
        return alphaLanes.length + betaLanes.length;
    }

    public int getNumAlphaLanes() {
        return alphaLanes.length;
    }

    public int getNumBetaLanes() {
        return betaLanes.length;
    }
    ...
    /**
     * Draws this Road, with all it's DriveLanes and Roadusers onto a Graphics
     * object, mostly a View
     *
    ...
    public Shape paintDrivelaneSegment(Graphics g, Point p1_1, Point p1_2, Point p2_1, Point p2_2, double a1, double a2, DriveLaneTemp dl) {
        switch (dl.getType()) {
            ...
            case 3:
                ...
                break;      // Cars and Busses only (default, geloof ik)
            case 4:
                g.setColor(new Color(230, 255, 230));
                break;      // Bicycles only
            case 5:
                g.setColor(new Color(230, 255, 255));
                break;      // Cars and Bicycles only
            case 6:
                g.setColor(new Color(255, 255, 230));
                break;      // Busses and Bicycles only
            default:
     ...
     * @param pos The position of the Roaduser relative to the start of the
     * segment
     */
...
//			double a = ((Double)o[1]).doubleValue();
            ...
            x.printStackTrace();
            System.out.println("Exception (Road): " + x.getMessage());
        }
    }
...
// draw sign
        ...
        if (d.getNodeLeadsTo() instanceof SpecialNode) {
            return;
        }
...
// draw directions
        ...
        if (tars[0]) // left
        {
            switch ((int) Math.toDegrees(CurveUtils.normalize(a))) {
                case (90):
                    g.drawLine(q1.x, q1.y, q1.x + 3, q3.y);
                    break;
                case (0):
                    g.drawLine(q1.x, q1.y, q3.x, q1.y + 3);
                    break;
                case (360):
                    g.drawLine(q1.x, q1.y, q3.x, q1.y + 3);
                    break;
