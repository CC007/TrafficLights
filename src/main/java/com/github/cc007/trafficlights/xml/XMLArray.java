...
/**
 * Class to convert arrays of arbitrary dimensions from/to XML : Accepted
 * componenttypes : - int,float,double,boolean,byte and their respective
 * encapsulator classes, Strings and objects that accept XMLSerializable
 */
public class XMLArray implements XMLSerializable { // The data itself
     ...
     *
     * @param parentName The XML name of the object that is going to save this
     * array.
     * @returns The new XMLArray
     */
    public XMLArray() { 	// Empty
    }

    /**
     * Empty constructor for loading
     *
     * @param parentName The XML name of the object that is going to load this
     * array.
     ...
     * objects (this parameter is only meaningful for XMLArrays that directly or
     * indirectly contain XMLSerializable objects.)
     * @returns A new XMLArray which can be used for loading
     */
    ...
    /**
     * Return the result array,ArrayList or LinkedList as an object. The
     * XMLArray has to be loaded first.
     *
    ...
    public Object getResult() throws XMLInvalidInputException {
        if (data == null) {
            throw new XMLInvalidInputException("Tried to get result from XMLArray when it was not loaded in "
                    + parentName);
        }
        ...
        throw new XMLInvalidInputException("Internal problem in XMLArray: result has unknown container ("
                + containerType + ")");
    }
    ...
    /**
     * Return the result array,ArrayList or LinkedList as an object. The
     * XMLArray has to be loaded first. First checks if the result class equals
     * the parameter class.
     *
     * @param expectedClass The class with which the result has to match
     * @throws XMLInvalidInputException If the XMLArray was not loaded, if there
     * is an internal problem or if the result class doesn't match the parameter
     * class.
     * @returns The array which has been parsed
     */
    public Object getResult(Class expectedClass) throws XMLInvalidInputException {
        ...
        if (result.getClass().equals(expectedClass)) {
            ...
            throw new XMLInvalidInputException("XMLArray could not return result : expected class "
     ...
     *
     * @param array An array to save (ArrayLists and LinkedLists are also
     * accepted)
    ...
    /**
     * Load an arbitrary array. Instantiate child objects via the parameter
    ...
    @Override
    public void load(XMLElement myElement, XMLLoader loader) throws XMLTreeException, IOException, XMLInvalidInputException { // Create array
        ...
        try {
            ...
            throw new XMLInvalidInputException("A new XMLArray couldn't create an instance of its data class:"
                    + className + " ,reason :" + e);
        }
        // Populate it
        ...
        for (int x = 0; x < length; x++) {
            ...
            switch (componentType) {
                case INT:
                    Array.setInt(data, x, loadObject.getIntValue());
                    break;
                case DOUBLE:
                    Array.setDouble(data, x, loadObject.getDoubleValue());
                    break;
                case FLOAT:
                    Array.setFloat(data, x, loadObject.getFloatValue());
                    break;
                case BOOLEAN:
                    Array.setBoolean(data, x, loadObject.getBooleanValue());
                    break;
                case INT_CLASS:
                    Array.set(data, x, loadObject.getIntClassValue());
                    break;
                case DOUBLE_CLASS:
                    Array.set(data, x, loadObject.getDoubleClassValue());
                    break;
                case FLOAT_CLASS:
                    Array.set(data, x, loadObject.getFloatClassValue());
                    break;
                case BOOLEAN_CLASS:
                    Array.set(data, x, loadObject.getBooleanClassValue());
                    break;
                case XMLSERIALIZABLE:
                    Array.set(data, x, loadObject.getObjectValue());
                    break;
                case OTHER_ARRAY:
                    Array.set(data, x, loadObject.getObjectValue());
                    break;
                case BYTE:
                    Array.setByte(data, x, loadObject.getByteValue());
                    break;
                case BYTE_CLASS:
                    Array.set(data, x, loadObject.getByteClassValue());
                    break;
            }
        }
    }
    ...
    public void saveChilds(XMLSaver saver) throws XMLTreeException, IOException, XMLCannotSaveException {
        ...
        for (int x = 0; x < Array.getLength(data); x++) {
            ...
            switch (componentType) {
                case INT:
                    toSave = (new XMLArrayElement(Array.getInt(data, x)));
                    break;
                case DOUBLE:
                    toSave = (new XMLArrayElement(Array.getDouble(data, x)));
                    break;
                case FLOAT:
                    toSave = (new XMLArrayElement(Array.getFloat(data, x)));
                    break;
                case BOOLEAN:
                    toSave = (new XMLArrayElement(Array.getBoolean(data, x)));
                    break;
                case INT_CLASS:
                    toSave = (new XMLArrayElement((Integer) Array.get(data, x)));
                    break;
                case DOUBLE_CLASS:
                    toSave = (new XMLArrayElement((Double) Array.get(data, x)));
                    break;
                case FLOAT_CLASS:
                    toSave = (new XMLArrayElement((Float) Array.get(data, x)));
                    break;
                case BOOLEAN_CLASS:
                    toSave = (new XMLArrayElement((Boolean) Array.get(data, x)));
                    break;
                case XMLSERIALIZABLE:
                    toSave = (new XMLArrayElement((XMLSerializable) ((Array.get(data, x)))));
                    break;
                case OTHER_ARRAY:
                    toSave = (new XMLArrayElement(new XMLArray(Array.get(data, x), parentName)));
                    break;
                case BYTE:
                    toSave = (new XMLArrayElement(Array.getByte(data, x)));
                    break;
                case BYTE_CLASS:
                    toSave = (new XMLArrayElement((Byte) Array.get(data, x)));
                    break;
            }
            if (toSave == null) {
    ...
    class XMLArrayElement implements XMLSerializable {
         ...
         * type
         *
         * @param expectedType The type which must be equal to the the type of
         * this XMLArrayElement
        ...
        void assertType(int expectedType) throws XMLInvalidInputException {
            if (type != expectedType) {
                throw new XMLInvalidInputException("Problem with extracting result from a XMLArrayElement :"
                ...
                XMLTreeException, IllegalAccessException // Lots of things that can go wrong
        {
            ...
            if (watson == null || XMLArray.class.equals(request)) {
                ...
                throw new ClassNotFoundException("Problem in XMLArrayElement : my InstantiationAssistant can't create "
        ...
        public void load(XMLElement myElement, XMLLoader loader) throws XMLTreeException, IOException, XMLInvalidInputException {
            ...
            if (!nullValue
                ...
                try {
                    ...
                    throw new XMLInvalidInputException("Something went wrong while trying to instantiate a new"
        ...
        public void setParentName(String parentName) throws XMLTreeException {
            throw new XMLTreeException("XMLArrayElement does not support setParentName. It uses the "
