...
class XMLInputReader extends InputStreamReader {
    ...
    /**
     * Read a string from the file until we encounter a specific character.
     *
     * @param c The specific char
     * @param including True if the char should be included in the string, false
    ...
    /**
     * Move the buffer to the first occurence of a certain char in the file.
     *
     * @param c The specific char
     * @param including True if the buffer should be moved to one position after
     * the char, false if it should be positioned at the char itself.
     * @throws IOException If a new tag cannot be read because of an IO error
     */
    protected void ignoreUntilChar(char c, boolean including) throws IOException {
        try {
            ...
            Logger.getLogger(XMLInputReader.class.getName()).log(Level.SEVERE, null, e);
        }
    }
