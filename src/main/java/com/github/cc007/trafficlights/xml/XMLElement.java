...
public class XMLElement {
     ...
     * @throws NoSuchElementIndex If an attribute with that name cannot be
     * found.
    ...
    protected int getAttributeIndex(String name) throws NoSuchElementException {
        ...
        throw new NoSuchElementException("Could not find XML attribute named " + name);
    }
    ...
    /**
     * @return a string which represents the XML tag which opens this element in
     * the XML file
     */
    ...
    public static XMLElement parse(String string) throws XMLTreeException {
        ...
        if (string.startsWith("</")) {
            throw new XMLTreeException("XMLElement.parse cannot handle close tags: " + original);
        }
        if (!(string.startsWith("<") && string.endsWith(">"))) {
            throw new XMLTreeException("Invalid XML tag (no < or > ) :" + original);
        }
        string = string.substring(1, string.length() - 1);
        t = new StringTokenizer(string, " ");
        if (!t.hasMoreTokens()) {
            throw new XMLTreeException("XML tag without valid name " + original);
        }
        XMLElement result = new XMLElement(t.nextToken());
        while (t.hasMoreTokens()) {
            a = new StringTokenizer(t.nextToken(), "=");
            if (a.countTokens() != 2) {
                throw new XMLTreeException("Wrong attribute in XML tag :" + original);
            }
            attName = a.nextToken();
            attValue = a.nextToken();
            if (!(attValue.startsWith("\"") && attValue.endsWith("\""))) {
                throw new XMLTreeException("Wrong attribute value in XML tag (no quotes?) :" + attValue + "/" + original);
            }
