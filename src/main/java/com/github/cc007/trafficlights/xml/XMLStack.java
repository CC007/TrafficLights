...
/**
 * This class is used by the XMLLoader and the XMLSaver to remember where the
 * parser is saving/loading in the XML tree. XMLStack is an extension of the
 * normal java.util.Stack.
 */
public class XMLStack extends Stack<Object> {
    ...
    public void assertIsCurrentParent(String s) throws XMLTreeException {
        ...
        if (("").equals(s)) {
            throw new XMLTreeException("Main parser tried to access the root of the XML tree when the "
                    + "XML stack was not empty. XML Stack dump :" + getBranchName());
        } else {
            throw new XMLTreeException("The XMLSerializable :" + s + ": tried to access the "
    ...
    public boolean isARootParent(String s) {
        return (("").equals(s) && super.isEmpty()) || s.length() == 0
                ...
                || XMLUtils.getGenericName(branchName, false).startsWith(XMLUtils.getGenericName(s, false));
    }
     ...
     */
    protected final void updateBranchName() {
