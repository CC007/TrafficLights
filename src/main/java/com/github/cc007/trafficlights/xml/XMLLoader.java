 ...
 * provides methods which XMLSerializable objects can use to retrieve their
 * child objects.
 */
public class XMLLoader {
     ...
     * @throws XMLInvalidInputException If the XML file contains invalid values.
     */
     ...
     * @throws XMLInvalidInputException If this XMLSerializable encounters
     * invalid input or cannot load itself for another reason.
     */
     ...
     * @param it The Iterator of XML serializables
     * @throws IOException If the XMLLoader cannot read from the XML file
     * because of an IO error.
     * @throws XMLTreeException If something goes wrong while the parser is
     * building the XML tree structure.
     * @throws XMLInvalidInputException If one of the XMLSerializables
     * encounters invalid input or cannot load itself for another reason.
     * @throws ClassCastException If the enumeration contains an object that is
     * not XMLSerializable
     */
    public void loadIterator(XMLSerializable parent, Iterator it) throws XMLTreeException, XMLInvalidInputException, IOException, ClassCastException {
        while (it.hasNext()) {
            load(parent, (XMLSerializable) (it.next()));
        }
    }
    ...
    protected void lookAhead() throws XMLTreeException, IOException {
        ...
        while (nextElement == null && !atEOF) { // Read tag 
            try {
                ...
                if (!stack.isEmpty()) {
                    throw new XMLTreeException("EOF at non-terminated tree. File is probably truncated.");
                } else {
            ...
            if (tag.startsWith("</")) {
                ...
                if (!tag.equals(ref)) {
                    throw new XMLTreeException("Unbalanced tags while loading XML file :" + ref + "/" + tag);
                }
            } // And begin tag
            else {
     ...
     * because of an IO error.
     * @throws XMLTreeException If something goes wrong while the parser is
     * building the XML tree structure or if the next element has another
     * parent.
     */
    public XMLElement getNextElement(XMLSerializable parent) throws XMLTreeException, IOException {
        ...
        if (nextElement == null) {
            throw new XMLTreeException("Can't find more XMLElements inside " + parent.getXMLName());
        }
     ...
     * @return A boolean value which indicates if another element can be
     * fetched.
     */
     ...
     * because of an IO error.
     * @throws XMLTreeException If something goes wrong while the parser is
     * building the XML tree structure or if the next element in the file has
     * another name/parent.
     */
    public XMLElement getNextElement(XMLSerializable parent, String name) throws XMLTreeException, IOException {
        ...
        if (name == null || !name.equals(nextElement.getName())) {
            throw new XMLTreeException("Can't find next element named " + name + " in parent "
    ...
    /**
     * Checks if an XMLSerializable can get an element with a specific name from
     * the file.
     *
     * @param parent The XMLSerializable that wants to request the next element
    ...
    /**
     * Tranfer the contents of the nextElement buffer to a XMLSerializable and
     * fill the buffer.
     *
     ...
     * less the enumeration form of getNextElement(parent). WARNING : using more
     * than one of these enumerations simultaneously may result in unpredictable
     * behaviour.
     *
     * @param parent The object which is requesting the enumeration
     * @return An enumeration of XMLElements
     */
    public Iterator getIterator(XMLSerializable parent) {
        return new XMLLoadIterator(parent);
    }

    /**
     * Returns an enumeration of XML elements with a specific name which can be
     * loaded. More or less the enumeration form of getNextElement(parent,name).
     * WARNING : using more than one of these enumerations simultaneously may
     ...
     * @return An enumeration of XMLElements
     */
    public Iterator getIterator(XMLSerializable parent, String tagName) {
        return new XMLLoadIterator(parent, tagName);
    }
    ...
    /**
     * The iterator which the getIterator methods return. 
     */
    class XMLLoadIterator implements Iterator {

        boolean hasName;
        String name;
        XMLSerializable parent;

        public XMLLoadIterator(XMLSerializable parent) {
            this.parent = parent;
            hasName = false;
        }

        public XMLLoadIterator(XMLSerializable parent, String name) {
            this.parent = parent;
            hasName = true;
            this.name = name;
        }
